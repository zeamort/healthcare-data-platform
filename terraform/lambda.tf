...
# Deployment packages are uploaded to the Lambda S3 bucket by scripts/package_lambdas.sh.

locals {
  lambda_s3_bucket = aws_s3_bucket.lambda.id
  lambda_s3_prefix = "lambda"
...
resource "aws_lambda_layer_version" "psycopg2" {
  ...
  s3_key              = "${local.lambda_s3_prefix}/psycopg2-layer.zip"
...
resource "aws_lambda_function" "schema_init" {
  ...
  s3_bucket = local.lambda_s3_bucket
  s3_key    = "${local.lambda_s3_prefix}/schema_init.zip"
...
resource "aws_lambda_function" "etl_s3_to_rds" {
  ...
  s3_bucket = local.lambda_s3_bucket
  s3_key    = "${local.lambda_s3_prefix}/etl_s3_to_rds.zip"
...
resource "aws_lambda_function" "etl_rds_to_redshift" {
  ...
  s3_bucket = local.lambda_s3_bucket
  s3_key    = "${local.lambda_s3_prefix}/etl_rds_to_redshift.zip"
  ...
  environment {
    variables = merge(local.rds_env, local.redshift_env, {
      ML_CLUSTERING_FUNCTION   = "${local.name_prefix}-ml-clustering"
      ML_RISK_SCORING_FUNCTION = "${local.name_prefix}-ml-risk-scoring"
...
resource "aws_lambda_function" "stream_consumer" {
  ...
  s3_bucket = local.lambda_s3_bucket
  s3_key    = "${local.lambda_s3_prefix}/stream_consumer.zip"
...
resource "aws_lambda_function" "ml_clustering" {
  function_name = "${local.name_prefix}-ml-clustering"
  role          = aws_iam_role.lambda_execution.arn
  ...
  s3_bucket = local.lambda_s3_bucket
  s3_key    = "${local.lambda_s3_prefix}/ml_clustering.zip"
  ...
  environment {
    variables = merge(local.redshift_env, {
      ML_MODULE = "ml_clustering"
    })
  }

  tags = { Name = "${local.name_prefix}-ml-clustering" }
}

# ── ML: Risk Scoring ───────────────────────────────────

resource "aws_lambda_function" "ml_risk_scoring" {
  function_name = "${local.name_prefix}-ml-risk-scoring"
  role          = aws_iam_role.lambda_execution.arn
  handler       = "ml_handler.lambda_handler"
  runtime       = local.lambda_runtime
  timeout       = 900
  ...
  s3_bucket = local.lambda_s3_bucket
  s3_key    = "${local.lambda_s3_prefix}/ml_risk_scoring.zip"
  ...
  environment {
    variables = merge(local.redshift_env, {
      ML_MODULE = "ml_risk_scoring"
    })
  }

  tags = { Name = "${local.name_prefix}-ml-risk-scoring" }
}
...
resource "aws_lambda_function" "ml_comorbidity" {
  ...
  s3_bucket = local.lambda_s3_bucket
  s3_key    = "${local.lambda_s3_prefix}/ml_comorbidity.zip"
...
resource "aws_cloudwatch_log_group" "lambda_logs" {
  for_each = toset([
    ...
    "stream-consumer", "ml-clustering", "ml-risk-scoring", "ml-comorbidity",
  ])
